# Navigation Debug Deep Dives

이 폴더는 ROS2 Navigation 실습 중 실제 디버깅에서 자주 부딪히는 주제를 따로 모은 심화 문서입니다.

기본 개념 심화 문서가 ROS2 Navigation 전체에서 반복되는 개념을 다룬다면, 이 폴더는 실패 원인을 분류하고 확인하는 기준을 다룹니다.
...
4. Navigation 실패 원인을 topic/frame/costmap/controller/BT 기준으로 분류하는 방법
```
...
이런 문제는 명령어만 외워서는 해결하기 어렵습니다. `topic`, `frame`, `TF`, `costmap`, `controller`, `Behavior Tree`의 역할을 나눠서 확인해야 합니다.
...
이 폴더는 코드 변경 내역이 아니라, Navigation 실패 원인을 분류하기 위한 문서입니다.
...
가장 먼저 확인할 주제는 `rosbag topic remap vs frame_id`입니다.
...
DWB와 Behavior Tree는 튜닝 전문가 수준으로 들어가기보다, Nav2가 왜 `/robot_ns/cmd_vel`을 만들거나 만들지 못하는지 판단할 수 있는 수준으로 정리합니다.
