...
이 문서는 학습 정리에서 중요한 구분을 다룬다.

```text
내가 직접 작성한 것인가?
내가 설정한 것인가?
외부 ROS2 패키지가 제공한 것인가?
```

이 구분이 중요하다. 그래야 나중에 “내가 구현했다”고 과장하지 않고, 정확히 무엇을 이해하고 설정했는지 말할 수 있다.

---

## 1. 큰 구분

| 영역 | 내가 한 것 | 외부 패키지가 해준 것 |
|---|---|---|
...
```text
내가 직접 Python/C++/XML/YAML 파일을 만들어 특정 동작을 정의한 것
```
...
이 표현은 현재 학습 정리 단계에는 과장이다.

---

## 4. 학습 정리에서의 올바른 초점

현재 문서의 목적은 “내가 다 만들었다”가 아니다.

정확한 초점:

```text
- 외부 패키지가 무엇을 해주는지 이해했다.
- 내가 어떤 topic/frame/parameter를 맞춰야 하는지 이해했다.
- 실행 결과를 어떻게 검증하는지 배웠다.
- 문제가 생겼을 때 어디부터 확인해야 하는지 정리했다.
```

이 기준이 나중에 진짜 2주 프로젝트를 할 때도 중요하다.

---

## 5. 나중 프로젝트로 이어질 수 있는 부분

나중에 실제 개인/팀 프로젝트를 할 때 직접 구현 또는 확장하기 좋은 부분:
...
이런 부분을 추가하면 학습 정리에서 프로젝트로 자연스럽게 넘어갈 수 있다.
