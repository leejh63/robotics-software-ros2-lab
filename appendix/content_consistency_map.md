# Content Consistency Map

이 문서는 저장소 안에서 같은 개념이 여러 번 반복될 때 어떤 문서를 기준으로 볼지 정리한 기준표다.

학습 노트 특성상 Day별 문서에는 같은 개념이 여러 번 등장한다. 반복 자체는 문제는 아니지만, GitHub 업로드용 문서에서는 오래된 설명과 최신 설명이 충돌하지 않도록 기준 문서를 정해두는 것이 좋다.
