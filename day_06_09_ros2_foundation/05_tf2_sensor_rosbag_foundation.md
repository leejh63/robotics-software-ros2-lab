...
## 3. odom_simul.py - 동적 TF 발행

`odom_simul.py`는 20 Hz 주기로 아래 TF를 발행한다.
...
## 4. tf_tree_simul.py - TF tree 구조 연습

`tf_tree_simul.py`는 아래와 같은 tree를 만든다.
...
`map_robot_ns -> odom_robot_ns`는 static transform으로 한 번 발행한다.
...
`odom_robot_ns -> base_link_robot_ns`, `base_link_robot_ns -> m_lee`, `base_link_robot_ns -> k_lee`는 동적 transform으로 계속 발행한다.
...
```text
target_frame = base_link_robot_ns
source_frame = camera_link_lee
```
...
이번 코드에서 `imagePlee.py`는 다음처럼 설정한다.
...
`tf_broad_yolo.py`는 detection array의 header frame을 parent frame으로 쓴다.
...
`tf_broad_yolo.py`는 bbox 중심을 이용해서 object frame을 만든다.
