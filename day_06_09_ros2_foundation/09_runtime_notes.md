...
이 문서는 Day 06~09 ROS2 학습 코드에서 실행 중 헷갈리기 쉬운 부분을 정리한다. 실행 문제를 패키지 의존성, parameter 반영 여부, frame 이름, GUI 환경 문제로 분리해서 확인하는 것이 목적이다.
...
하지만 `this_test/package.xml`에는 `geometry_msgs` 의존성이 보이지 않는다.

확인 기준:

```text
환경에 geometry_msgs가 이미 설치되어 있으면 실행될 수 있다.
재현 가능한 패키지 기준으로는 package.xml에 geometry_msgs 의존성을 추가하는 것이 맞다.
```
...
```text
runtime에서 바로 실패하지 않을 수도 있지만, 패키지 의존성 문서화 기준으로는 명시하는 것이 좋다.
```
...
model_path는 parameter로 명시하는 편이 안전하다.
```
...
parameter 선언과 실제 사용 위치가 일치하는지 확인해야 한다.
```
...
그런데 `py_launch_example` 자체에 config 폴더가 보이지 않고, `lee_ep_launch.py`는 `camera_pkg`의 config를 사용한다.
...
py_launch_example의 setup.py에 남아 있는 config 설치 규칙은 실제 사용 여부를 확인해야 한다.
```
...
launch argument 이름과 실제 실행 프로그램이 다를 수 있다.
```
...
```text
특정 workspace 경로에서는 동작할 수 있다.
다른 컴퓨터나 다른 workspace 경로에서는 깨질 수 있다.
RViz config는 패키지 share에 넣고 get_package_share_directory()로 찾는 방식이 더 재현성이 좋다.
```
...
```text
- package.xml 의존성 명시
- launch argument 이름과 실제 실행 프로그램 일치
- YOLO model_path 절대경로 또는 parameter 처리
- RViz config 경로를 package share 기준으로 변경
- frame 이름 기준 정리
- YOLO image node와 custom message node의 node name 분리
```
