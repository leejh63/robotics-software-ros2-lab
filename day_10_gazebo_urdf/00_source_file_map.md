# 00. Source File Map - lee_robot_description 기준

이 문서는 Day 10에서 사용하는 실제 파일들이 어떤 역할을 하는지 정리한 지도이다.
...
Day 10만 보면 Gazebo/URDF/RViz 관련 의존성이 핵심이고, Day 11~13까지 이어지면서 `slam_toolbox`, `nav2_*` 의존성이 사용된다.
...
World 파일은 로봇 자체가 아니라 로봇이 돌아다닐 환경을 정의한다.
...
이름은 `wall_follower`지만 현재 로직은 벽 따라가기라기보다 아래에 가깝다.
