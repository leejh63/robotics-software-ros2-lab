...
4. `/robot_ns/joint_states`가 필요한 이유는 무엇인가?
5. `/tf`를 `/robot_ns/tf`로 remap하면 어떤 장점과 단점이 있는가?
...
1. Gazebo plugin이 없다면 `/robot_ns/scan`, `/robot_ns/odom`, `/robot_ns/image_raw`가 자동으로 생기는가?
2. diff drive plugin은 어떤 topic을 구독하고 어떤 topic을 발행하는가?
3. LiDAR plugin의 `<frame_name>base_scan</frame_name>`은 무엇을 의미하는가?
4. `/robot_ns/scan`과 `base_scan`의 차이는 무엇인가?
5. camera plugin이 발행하는 `/robot_ns/image_raw`는 Day 06~09의 OpenCV/YOLO 노드와 어떻게 연결될 수 있는가?
...
3. `spawn_entity.py`의 `-entity turtlebot_lee`는 ROS node 이름인가, Gazebo 모델 이름인가?
...
4. teleop과 회피 노드를 동시에 켜면 왜 `/robot_ns/cmd_vel` 충돌이 생기는가?
...
2. `/robot_ns/scan`이 있어도 TF가 맞지 않으면 SLAM이 어려운 이유는 무엇인가?
3. Gazebo에서 만든 `/robot_ns/odom`은 실제 로봇의 wheel encoder odom과 어떤 점에서 비슷하고 어떤 점에서 다른가?
