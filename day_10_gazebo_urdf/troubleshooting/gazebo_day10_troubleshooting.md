...
4. /tf를 /robot_ns/tf로 remap했다면 RViz2도 같은 remap을 받았는가?
```
...
```bash
ros2 topic echo /robot_ns/robot_description --once
...
2. /robot_ns/robot_description topic이 비어 있음
3. xacro 변환 실패
...
## 4. `/robot_ns/cmd_vel`을 보내도 로봇이 안 움직임
...
Publisher가 여러 개면 `/robot_ns/cmd_vel` 충돌 가능성이 있다.
