...
D항은 변화율에 반응한다. 흔들림을 줄이는 감쇠 역할을 한다.
...
D가 너무 크면 노이즈나 작은 변화에 민감해질 수 있다.

## I gain 추가

I항은 오래 남는 offset을 줄이는 데 사용한다.

```bash
ros2 param set /pid_arm_controller ki 0.1
```

I가 너무 크면 적분항이 쌓여서 overshoot가 커질 수 있다. 이 구현은 `integral_limit`과 saturation 시 적분 되돌리기를 통해 windup을 줄입니다.

## 중력 보상

팔은 중력 때문에 특정 각도에서 아래로 처질 수 있다. `gravity_gain`을 사용하면 간단한 feedforward 보상을 추가할 수 있다.
...
이 구현은 현재 각도 기준으로 아래 값을 더한다.

```text
gravity_comp = -gravity_gain * cos(current_position)
```

부호가 반대로 느껴지면 `gravity_gain`을 음수로 줄 수 있다.
...
값이 이상하면 gain을 바꾸기 전에 먼저 `/joint_states`와 controller 상태를 확인한다.
