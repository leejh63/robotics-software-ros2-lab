...
- 실제 rosbag DB 파일
- 대용량 영상/이미지 원본
- build/, install/, log/ 같은 colcon 빌드 산출물
- 개인 IP, 실제 hostname, 실제 사용자 홈 경로
- 작업 인계용 내부 관리 문서
```

이 저장소는 실행 산출물 저장소가 아니라 학습 노트 저장소입니다.

---

## 업로드 전 확인

GitHub에 올리기 전에는 아래 문서를 확인합니다.
...
```bash
git add .
git commit -m "docs: add ROS2 navigation study notes"
```
